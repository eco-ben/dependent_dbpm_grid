...
# Identify search volumes that resulted in successful equilibrium runs
out_folder <- "/g/data/vf71/fishmip_outputs/ISIMIP3a/fao_lme_outputs"
global_success <- read_parquet(file.path(out_folder, 
    # "successful_equilibrium_runs_inputs-outputs_global.parquet")) |> 
    "successful_equilibrium_runs_inputs-outputs_no-trunc_global.parquet")) |> 
...
for(f in fao_lme){
  ...
  # }
  # equilibrium_folder <- file.path(out_folder, f, "equilibrium_run")
  equilibrium_folder <- file.path(out_folder, f, "equilibrium_run_no-trunc")
  
  # Folder to store simulation results
  # results_folder <- file.path(out_folder, f, "no-fishing_run")
  results_folder <- file.path(out_folder, f, "no-fishing_run_no-trunc")
  ...
  sv_vals <- global_success |> 
    filter(region ==  unique(dbpm_inputs$region)) |> 
    arrange(search_vol)
  
  # Parallelising equilibrium runs per region of interest
  future_walk(sv_vals$search_vol, ~{
    sv <- .x
    # Load results from equilibrium run
    init_results <- read_json(list.files(
      equilibrium_folder, pattern = paste0("init_dbpm.*vol-", sv, "_.*json"),
    ...
    fish_param <- data.frame("region" = str_replace(str_to_upper(f), "-", " "),
                             ...
                             "fminx_v" = 0, "search_vol" = sv)
    
    ts_stablespin <- init_results$params$numb_time_steps
    
    end_fn <- paste0("_", f, "_searchvol-", sv, "_", 
